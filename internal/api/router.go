...
func NewRouter(queries *storage.Queries, bus *events.Bus, jwtSecret string) http.Handler {
	...
	protected.HandleFunc("POST /api/v1/sample-results", h.createSampleResult)
	protected.HandleFunc("PATCH /api/v1/sample-results/{id}/review", h.reviewSampleResult)
	protected.HandleFunc("PATCH /api/v1/sample-results/{id}/approve", h.approveSampleResult)
	protected.HandleFunc("POST /api/v1/organizations/{org_id}/sample-results/import", h.importSampleResults)
